  ...
  <div>
    ...
    <div class="company">
      ...
      <el-table ref="multipleTable" :data="tableData3" border style="width: 100%">
        ...
        <el-table-column label="注册资金" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-input v-model="scope.row.amount" placeholder="请输入注册资金" @blur="InputNumber(scope.row, 'amount')" />
          </template>
        </el-table-column>
        <el-table-column label="注册文件" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-upload class="upload-demo" :action="action" :data="uploadData" :on-preview="handlePreview" :on-remove="function(file,fileList){return handleRemove(file,fileList,scope.$index)}" :on-success="function(res,file,fileList){return handleSuccess(res,file,fileList,scope.$index)}" multiple :limit="1" :file-list="fileList[scope.$index]">
